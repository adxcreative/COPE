...
# Last Change:  2022-11-10 13:03:55
import numpy as np
...
class TextRelevanceLoss(nn.Module):
    ...
    def forward(self, image_emb, text_emb, is_xbm=False):
        ...
        if image_emb.dim() == 2:
            if text_emb.dim() == 2:
                ...
                sim_i2t = torch.matmul(image_emb.unsqueeze(1).unsqueeze(1), text_emb.permute(0,2,1)).squeeze()
                sim_i2t, _ = sim_i2t.max(-1)
                sim_t2i = torch.matmul(text_emb.unsqueeze(1), image_emb.unsqueeze(-1)).squeeze()
                sim_t2i, _ = sim_t2i.max(-1)
            else:
                raise ValueError
        elif image_emb.dim() == 3:
            if text_emb.dim() == 2:
                sim_i2t = torch.matmul(image_emb.unsqueeze(1), text_emb.unsqueeze(-1)).squeeze()
                sim_i2t, _ = sim_i2t.max(-1)
                sim_t2i = torch.matmul(text_emb.unsqueeze(1).unsqueeze(1), image_emb.permute(0,2,1)).squeeze()
            elif text_emb.dim() == 3:
                sim_i2t = torch.matmul(image_emb.unsqueeze(1), text_emb.permute(0,2,1))
                sim_i2t, _ = sim_i2t.max(-1).values.max(-1)
                sim_t2i = torch.matmul(text_emb.unsqueeze(1), image_emb.permute(0,2,1))
                sim_t2i, _ = sim_t2i.max(-1).values.max(-1)
            else:
                raise ValueError
        else:
            raise ValueError

        logits_per_image = logit_scale * sim_i2t
        logits_per_text = logit_scale * sim_t2i
        ...
        #print(batch_size)
        ground_truth = torch.arange(batch_size).cuda()
